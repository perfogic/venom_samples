...
describe("Test Tokensale contract", async function () {
  ...
  describe("Contracts", async function () {
    ...
    it("Deploy contract", async function () {
      ...
      expect(await locklift.provider.getBalance(tokensale.address).then(balance => Number(balance))).to.be.eq(
        Number(toNano(1)),
      ); // remember, that we are reserving 1 venom on Tokensale contract
    });
    ...
    it("Send supply to tokensale wallet", async function () {
      ...
      const { value0: ownerWalletAddress } = await tokenRoot.methods
        ...
        .call();
      const ownerWallet = await locklift.factory.getDeployedContract("TokenWallet", ownerWalletAddress);
      ...
      const { value0: tokensaleWalletAddress } = await tokenRoot.methods
        .walletOf({
          answerId: 0,
          walletOwner: tokensale.address,
        })
        .call();
      const tokensaleWallet = await locklift.factory.getDeployedContract("TokenWallet", tokensaleWalletAddress);
    ...
    it("Buy tokens from tokensale contract", async function () {
      ...
      const { value0: buyerWalletAddress } = await tokenRoot.methods
        .walletOf({
          answerId: 0,
          walletOwner: buyerAccount.address,
        })
        .call();
      const buyerWallet = await locklift.factory.getDeployedContract("TokenWallet", buyerWalletAddress);
    ...
    it("Trying to buy more than supply", async function () {
      ...
      const { value0: buyerWalletAddress } = await tokenRoot.methods
        ...
        .call();
      expect(await locklift.provider.getBalance(buyerWalletAddress).then(balance => Number(balance))).to.be.eq(
        Number(0),
      );
